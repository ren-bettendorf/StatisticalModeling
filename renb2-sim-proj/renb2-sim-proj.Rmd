...
```
```{r}
hist(sim2_data$non_p, main = expression(paste("Non-Significant P-Value - ", sigma, " = 5")), prob = TRUE, breaks = 40)
x = sim2_data$non_p
curve(dunif(x), col='orange', add=TRUE, lwd=3)
```
```{r}
...
```{r}
barplot(table(factor(colnames(sim1_rmse_test)[apply(sim1_rmse_test, 1, function(x) which.min(x))], levels = colnames(sim1_rmse_test))), las = 2)
```
```{r}
...
```{r}
library(knitr)
mean_data = data.frame(sigma_1_train = apply(sim1_rmse_train, 2, mean), sigma_1_test = apply(sim1_rmse_test, 2, mean), sigma_2_train = apply(sim2_rmse_train, 2, mean), sigma_2_test = apply(sim2_rmse_test, 2, mean), sigam_3_train = apply(sim3_rmse_train, 2, mean), sigam_3_test = apply(sim3_rmse_test, 2, mean))
kable(t(mean_data), col.names=c("Model 1 Mean RMSE", "Model 2 Mean RMSE", "Model 3 Mean RMSE",
                                "Model 4 Mean RMSE", "Model 5 Mean RMSE", "Model 6 Mean RMSE",
                                "Model 7 Mean RMSE", "Model 8 Mean RMSE", "Model 9 Mean RMSE"))
```
...
```{r}
library(ggplot2)
power_data_sub = power_data[which(power_data$sigma == 1),]
power1 = power_data_sub[which(power_data_sub$n == 10),]
power2 = power_data_sub[which(power_data_sub$n == 20), c("n", "power")]
power3 = power_data_sub[which(power_data_sub$n == 30), c("n", "power")]

power_full = cbind(power1, power2, power3)
colnames(power_full) = c("sigma", "beta1", "n10", "power_n10", "n20", "power_n20", "n30", "power_n30" )

ggplot(data=power_full, aes(x = n10)) + geom_line(aes(x = n10, y = power_n10, color = "power_n10"), size=1) +   geom_line(aes(x = n10, y = power_n20, color = "power_n20"), size=1) +
  geom_line(aes(x = n10, y = power_n30, color = "power_n30"), size=1) +
...
```{r}
power_data_sub = power_data[which(power_data$sigma == 2),]
power1 = power_data_sub[which(power_data_sub$n == 10),]
power2 = power_data_sub[which(power_data_sub$n == 20), c("n", "power")]
power3 = power_data_sub[which(power_data_sub$n == 30), c("n", "power")]
...
```{r}
power_data_sub = power_data[which(power_data$sigma == 4),]
power1 = power_data_sub[which(power_data_sub$n == 10),]
power2 = power_data_sub[which(power_data_sub$n == 20), c("n", "power")]
power3 = power_data_sub[which(power_data_sub$n == 30), c("n", "power")]
